...
xdescribe('Integration Test => filesAffectedBy Queue ', () => {
  ...
  const newFilesClient: FilesAffectedByClient = new FilesAffectedByClient(
    prospect
  );
  ...
  it('Test mongo create filesAffectedBy queue', async () => {
    ...
    //When
    let createdFilesQueue: FilesAffectedByQueueModel = await filesQueueTDG.create(
      newFilesQueue,
      queueId
    );
  ...
  it('Test mongo update filesAffectedBy queue', async () => {
    ...
    let updatedUser: boolean = await filesQueueTDG.update(
      queueId,
      newFilesQueue
    );
