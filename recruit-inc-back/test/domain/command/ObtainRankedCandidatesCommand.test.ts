...
describe.only('Query created to fetch different filters', () => {
  it('Should return the right ranked query when single language is passed', async () => {
    ...
    // THEN
    expect('{"$addFields":{"totalLOCQueried":{"$sum":{"$map":{"input":"$tmp.gitProjectSummary.totalOutput","as":"totalOutput","in":{"$cond":{"if":{"$or":[{"$eq":["$$totalOutput.languageOrFramework","Java"]}]},"then":{"$add":[{"$sum":{"$map":{"input":"$$totalOutput.frameworks","as":"frameworks","in":{"$cond":{"if":{"$or":[]},"then":{"frameworkTarget":"$$frameworks.linesOfCode"},"else":0}}}}},"$$totalOutput.linesOfCode"]},"else":0}}}}},"totalCommitsQueried":{"$sum":{"$map":{"input":"$tmp.gitProjectSummary.totalOutput","as":"totalOutput","in":{"$cond":{"if":{"$or":[{"$eq":["$$totalOutput.languageOrFramework","Java"]}]},"then":{"$add":[{"$sum":{"$map":{"input":"$$totalOutput.frameworks","as":"frameworks","in":{"$cond":{"if":{"$or":[]},"then":{"frameworkTarget":"$$frameworks.numberOfCommits"},"else":0}}}}},"$$totalOutput.numberOfCommits"]},"else":0}}}}}}}').to.equal(JSON.stringify(actual));
  });
  ...
  it('Should return the right ranked query when a single framework is passed', async () => {
    ...
    // THEN
    expect('{"$addFields":{"totalLOCQueried":{"$sum":{"$map":{"input":"$tmp.gitProjectSummary.totalOutput","as":"totalOutput","in":{"$cond":{"if":{"$or":[{"$eq":["$$totalOutput.languageOrFramework","Javascript"]}]},"then":{"$add":[{"$sum":{"$map":{"input":"$$totalOutput.frameworks","as":"frameworks","in":{"$cond":{"if":{"$or":[{"$eq":["$$frameworks.technologyName","React"]}]},"then":{"frameworkTarget":"$$frameworks.linesOfCode"},"else":0}}}}},"$$totalOutput.linesOfCode"]},"else":0}}}}},"totalCommitsQueried":{"$sum":{"$map":{"input":"$tmp.gitProjectSummary.totalOutput","as":"totalOutput","in":{"$cond":{"if":{"$or":[{"$eq":["$$totalOutput.languageOrFramework","Javascript"]}]},"then":{"$add":[{"$sum":{"$map":{"input":"$$totalOutput.frameworks","as":"frameworks","in":{"$cond":{"if":{"$or":[{"$eq":["$$frameworks.technologyName","React"]}]},"then":{"frameworkTarget":"$$frameworks.numberOfCommits"},"else":0}}}}},"$$totalOutput.numberOfCommits"]},"else":0}}}}}}}').to.equal(JSON.stringify(actual));
  }); 
});
