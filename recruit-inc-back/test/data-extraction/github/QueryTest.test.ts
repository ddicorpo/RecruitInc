...
describe('Get User information', () => {
  ...
  it('Get a user by username,location and url', () => {
    return test.firstQuery('Montreal').then(response => {
      ...
      expect(JSON.parse(response).data.search.nodes[0].login).to.equal(
        'Suzylxx'
      );
      expect(JSON.parse(response).data.search.nodes[0].location).to.equal(
        'Montreal'
      );
      expect(JSON.parse(response).data.search.nodes[0].url).to.equal(
        'https://github.com/Suzylxx'
      );
    });
  });
});
