...
describe('Get commits from a user in a github repo', () => {
  ...
  it('Gets user commit hash', () => {
    return test
      .GetCommitsSpecificToUser(
        'MinistocksRework',
        'AyoubeAkaouch',
        'MDQ6VXNlcjI1MjUyMTIz'
      )
      .then(response => {
        ...
        expect(response['node']['oid']).to.equal(
          'eff56ed52a77dfb8020a7ab11468138c00a3284f'
        );
      });
  });
});
...
describe('Get files affected by a given commit', () => {
  it('should return the file path as well as the lines added and deleted of the files affected by a commit', async function() {
    nock('https://api.github.com')
      .get(
        '/repos/AyoubeAkaouch/MinistocksRework/commits/950e3b390cc8f569183cfbb5ec924537b083c191'
      )
    ...
    let result: {
      ...
      lineAdded: number;
      lineDeleted: number;
    }[] = await test.getFilesAffectedByCommit(
      'AyoubeAkaouch',
      'MinistocksRework',
      '950e3b390cc8f569183cfbb5ec924537b083c191'
    );
    ...
    expect(result[7].filePath).to.equal(
