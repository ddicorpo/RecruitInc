...
describe('Get User repositories', () => {
  ...
  it("Gets a user's repositories", () => {
    return test.firstQuery('MewtR').then(response => {
      expect(response).to.be.a('string');

      response = JSON.parse(response);
      //User has 6 repositories
      expect(response['data']['user']['repositories']['nodes'].length).to.equal(
        6
      );
      //Check a specific repository
      expect(
        response['data']['user']['repositories']['nodes'][3]['name']
      ).to.equal('MinistocksRework');
      expect(
        response['data']['user']['repositories']['nodes'][3]['owner']['login']
      ).to.equal('AyoubeAkaouch');
    });
  });
});
...
describe('Populates an IGithubUser with his repositories', () => {
  ...
  it('Populates an IGithubUser with his repositories', () => {
    return test.getUserRepos(user).then(response => {
      ...
      //Check a specific repository
      expect(user.dataEntry.projectInputs[3].projectName).to.equal(
        'MinistocksRework'
      );
