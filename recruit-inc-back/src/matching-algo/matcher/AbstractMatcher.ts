...
export abstract class AbstractMatcher {
  ...
  protected abstract package(
    codeOutput: ICodeOutput
  ): IFrameworkOutput | ILanguageOutput;
  ...
  protected processSourceFiles(): IProcessedSourceFile[] {
    ...
    for (const sourceFile of sourceFiles) {
      ...
      for (const matchingTarget of this.matchingConfig.matchingTargets) {
        const isFilenameMatchingFileToParse: boolean =
          filename === matchingTarget.sourceFileToParse;
  ...
  protected countCommitsAndLinesOfCode(basePath: string): ICodeOutput {
    ...
    for (const commit of commits) {
      ...
      let commitAnalysis: ICommitAnalysis = this.countNumberOfLinesInSingleFileCommits(
        singleFileCommits,
        basePath
      );
  ...
  private countNumberOfLinesInSingleFileCommits(
    commits: ISingleFileCommit[],
    basePath: string
  ): ICommitAnalysis {
    ...
    for (const commit of commits) {
      ...
      const isOfBasePath: boolean = this.isFilePathContainingBasePath(
        filePath,
        basePath
      );
      const isOfExtension: boolean = this.isFilepathOfExtension(filePath);
      const resultIntersection: string[] = IntersectionArrayString.intersection(
        commit.filePath.split('/'),
  ...
  private isFilePathContainingBasePath(
    filePath: string,
    basePath: string
  ): boolean {
