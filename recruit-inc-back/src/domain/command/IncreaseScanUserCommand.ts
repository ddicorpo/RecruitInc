...
export class IncreaseScanUserCommand extends AbstractCommand {
    ...
    public async increasedByOne(location:string): Promise<Boolean>{
        try{
            let targetCronStatus : ICronModel =
             await this.finder.findByLocation(location);
            ...
            const successful : Boolean = await this.cronTDG.update(targetCronStatus._id ,targetCronStatus);
