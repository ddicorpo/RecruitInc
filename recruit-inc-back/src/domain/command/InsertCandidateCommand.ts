...
export class InsertCandidateCommand extends AbstractCommand {
  ...
  private inStorageCommand: IsCandidateInStorageCommand = new IsCandidateInStorageCommand();
  ...
  public async insertCandidate(applicant: IApplicantModel): Promise<Boolean> {
    const isInsertCompleted: Boolean = await this.inStorageCommand.isCandidateIn(
      applicant.platformUsername
    );
    let commandStatus: boolean = false;
    try {
      if (isInsertCompleted) {
        //Grab the id of the object
        const foundApplicant: IApplicantModel = await this.applicantFinder.findByPlatformUsername(
          applicant.platformUsername
        );
        //Update Candidate with the new value
        applicant._id = foundApplicant._id;
        commandStatus = await this.applicantTDG.update(
          foundApplicant._id,
          applicant
        );
      } else {
        //Insert the new candidate
        const newApplicant: IApplicantModel = await this.applicantTDG.create(
          applicant
        );
        commandStatus =
          newApplicant.platformUsername === applicant.platformUsername;
      }

      if (commandStatus) {
        this.logActionCompleted(
          this.insertCandidate.name + ' inserted in Applicant table'
        );
      } else {
        this.logActionFailure(
          this.insertCandidate.name,
          'Error',
