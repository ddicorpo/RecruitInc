...
export class StackOverflowAPI {
  public queryProfileData(
    userId: string
  ): IStackOverFlowProfile | IStackOverFlowError {
    const preparedUrl: string =
      API_ROOT_URL + userId + '?order=desc&sort=reputation&site=stackoverflow';
    return this.queryUserData(userId, preparedUrl, 'queryProfileData');
  }

  public queryBadgesData(
    userId: string
  ): IStackOverFlowBadges | IStackOverFlowError {
    const preparedUrl: string =
      API_ROOT_URL + userId + '/badges?order=desc&sort=rank&site=stackoverflow';
   ...
   * @param userId
   */
  public queryNetworkData(
    userId: string
  ): IStackOverFlowNetwork | IStackOverFlowError {
