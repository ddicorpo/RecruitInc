...
export class StackOverflowQueries {
  async obtainProfileData(
    userId: string
  ): Promise<IStackOverFlowProfile | IStackOverFlowError> {
    return await new StackOverflowAPI().queryProfileData(userId);
  }
  async obtainBadgesData(
    userId: string
  ): Promise<IStackOverFlowBadges | IStackOverFlowError> {
    return await new StackOverflowAPI().queryBadgesData(userId);
  }
  async obtainNetworkData(
    userId: string
  ): Promise<IStackOverFlowNetwork | IStackOverFlowError> {
